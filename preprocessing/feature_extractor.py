...
Per-vertex (node) features, 8-dim:
    ...
    - curvature     : discrete Gaussian curvature (angle-defect), normalized
    - valence       : vertex degree (# adjacent vertices), normalized

Per-edge features, 8-dim:
    [length, dihedral_angle, curvature, valence, normal_difference,
     boundary, uv_seam, face_area]
    - length            : edge length (in the same normalized space as node xyz)
    - dihedral_angle    : angle (radians) between the two adjacent face normals
    - curvature         : mean vertex curvature of the two endpoints
    - valence           : mean vertex valence of the two endpoints
    - normal_difference : 1 - cos(dihedral_angle), in [0, 2]
    - boundary          : 1.0 if the edge belongs to a single face
    - uv_seam           : 1.0 if the edge lies on a UV island boundary
    - face_area         : mean area of the adjacent face(s)
"""
...
def _vertex_valence(mesh: trimesh.Trimesh) -> np.ndarray:
    ...
    max_val = max(1.0, float(valence.max()))
    return valence / max_val
...
def extract_edge_features(mesh: trimesh.Trimesh) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    edge_attr_unique = np.stack(
        [lengths, dihedral, edge_curvature, edge_valence, normal_diff, boundary, uv_seam, face_area],
        axis=1,
    ).astype(np.float32)
